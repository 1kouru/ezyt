...
(function () {
  ...
  function scriptPromptFor(titleDe, summaryRu) {
    ...
    return `напиши на эту тему\n${title}\n${summary}\nисторию, на +- 10.000 слов, важно сделать этот текст без воды, интересным и с сюжетными поворотами, чтобы зрителю хотелось дослушать. текст должен быть на том же языке, что и название. не разделяй на части, ты пишешь текст который сразу будет закинут в озвучку, пиши только текст без разделения на части\nне пиши слишком поэтично, напиши так чтобы обычному человеку было интересно слушать просто что произойдет.`;
  }
